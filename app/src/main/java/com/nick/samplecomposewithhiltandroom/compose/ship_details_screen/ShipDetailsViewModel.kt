...
class ShipDetailsViewModel @Inject constructor(application: Application) :
    ...
    private val _shipDetails = MutableStateFlow<ShipsEntity>(ShipsEntity())
    val shipDetails: StateFlow<ShipsEntity> = _shipDetails
    ...
    fun queryShipById(id: String) = launch {
        flow {
            val shipsEntity: ShipsEntity? = shipDetailsRepository.queryShipById(id)
            emit(shipsEntity)
